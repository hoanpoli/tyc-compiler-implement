...
def test_71_illegal_escape():
    assert Tokenizer('"a\\q"').get_tokens_as_string() == "Illegal Escape In String: a\\q"

def test_72_illegal_escape_mid():
    assert Tokenizer('"abc\\9xyz"').get_tokens_as_string() == "Illegal Escape In String: abc\\9"
...
def test_79_illegal_escape_hex():
    assert Tokenizer('"\\x01"').get_tokens_as_string() == "Illegal Escape In String: \\x"

def test_80_comment_then_error():
    assert Tokenizer('//cmt\n"a\\q"').get_tokens_as_string() == "Illegal Escape In String: a\\q"
...
def test_87_full_decl():
    assert Tokenizer("int x=1+2*3;").get_tokens_as_string() == "int,x,=,1,+,2,*,3,;,<EOF>"

def test_88_for_like():
    assert Tokenizer("for(i=0;i<10;i++)").get_tokens_as_string() == \
           "for,(,i,=,0,;,i,<,10,;,i,++,),<EOF>"
...
def test_100_mixed_all():
    assert Tokenizer('auto x=print("hi\\n");').get_tokens_as_string() == \
           "auto,x,=,print,(,hi\n,),;,<EOF>"
